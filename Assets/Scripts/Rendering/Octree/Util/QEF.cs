using Unity.Collections;
using Unity.Mathematics;
using static Unity.Mathematics.math;

namespace Core.Util
{
    public unsafe class QEF
    {
        const int SVD_NUM_SWEEPS = 10;
        const float PSUEDO_INVERSE_THRESHOLD = 0.1f;
        static void svd_mul_matrix_vec(float4* result, float3x3 a, float4 b)
        {
            (*result).x = dot(float4(a[0][0], a[0][1], a[0][2], 0.0f), b);
            (*result).y = dot(float4(a[1][0], a[1][1], a[1][2], 0.0f), b);
            (*result).z = dot(float4(a[2][0], a[2][1], a[2][2], 0.0f), b);
            (*result).w = 0.0f;
        }

        static void givens_coefs_sym(float a_pp, float a_pq, float a_qq, float* c, float* s)
        {
            if (a_pq == 0.0f)
            {
                *c = 1.0f;
                *s = 0.0f;
                return;
            }
            float tau = (a_qq - a_pp) / (2.0f * a_pq);
            float stt = sqrt(1.0f + tau * tau);
            float tan = 1.0f / ((tau >= 0.0f) ? (tau + stt) : (tau - stt));
            *c = rsqrt(1.0f + tan * tan);
            *s = tan * (*c);
        }

        static void svd_rotate_xy(float* x, float* y, float c, float s)
        {
            float u = *x; float v = *y;
            *x = c * u - s * v;
            *y = s * u + c * v;
        }

        static void svd_rotateq_xy(float* x, float* y, float* a, float c, float s)
        {
            float cc = c * c; float ss = s * s;
            float mx = 2.0f * c * s * (*a);
            float u = *x; float v = *y;
            *x = cc * u - mx + ss * v;
            *y = ss * u + mx + cc * v;
        }

        static void svd_rotate(float3x3 vtav, float3x3 v, int a, int b)
        {
            if (vtav[a][b] == 0.0f) return;

            float c, s;
            givens_coefs_sym(vtav[a][a], vtav[a][b], vtav[b][b], &c, &s);

            float x, y, z;
            x = vtav[a][a]; y = vtav[b][b]; z = vtav[a][b];
            svd_rotateq_xy(&x, &y, &z, c, s);
            vtav[a][a] = x; vtav[b][b] = y; vtav[a][b] = z;

            x = vtav[0][3 - b]; y = vtav[1 - a][2];
            svd_rotate_xy(&x, &y, c, s);
            vtav[0][3 - b] = x; vtav[1 - a][2] = y;

            vtav[a][b] = 0.0f;

            x = v[0][a]; y = v[0][b];
            svd_rotate_xy(&x, &y, c, s);
            v[0][a] = x; v[0][b] = y;

            x = v[1][a]; y = v[1][b];
            svd_rotate_xy(&x, &y, c, s);
            v[1][a] = x; v[1][b] = y;

            x = v[2][a]; y = v[2][b];
            svd_rotate_xy(&x, &y, c, s);
            v[2][a] = x; v[2][b] = y;
        }

        static void svd_solve_sym(NativeArray<float> a, float4* sigma, float3x3 v)
        {
            // assuming that A is symmetric: can optimize all operations for 
            // the upper right triagonal
            float3x3 vtav = new();
            vtav[0][0] = a[0]; vtav[0][1] = a[1]; vtav[0][2] = a[2];
            vtav[1][0] = 0.0f; vtav[1][1] = a[3]; vtav[1][2] = a[4];
            vtav[2][0] = 0.0f; vtav[2][1] = 0.0f; vtav[2][2] = a[5];

            // assuming V is identity: you can also pass a matrix the rotations
            // should be applied to. (U is not computed)
            for (int i = 0; i < SVD_NUM_SWEEPS; ++i)
            {
                svd_rotate(vtav, v, 0, 1);
                svd_rotate(vtav, v, 0, 2);
                svd_rotate(vtav, v, 1, 2);
            }

            *sigma = float4(vtav[0][0], vtav[1][1], vtav[2][2], 0.0f);
        }

        static float svd_invdet(float x, float tol)
        {
            return (abs(x) < tol || abs(1.0f / x) < tol) ? 0.0f : (1.0f / x);
        }

        static void svd_pseudoinverse(float3x3 o, float4 sigma, float3x3 v)
        {
            float d0 = svd_invdet(sigma.x, PSUEDO_INVERSE_THRESHOLD);
            float d1 = svd_invdet(sigma.y, PSUEDO_INVERSE_THRESHOLD);
            float d2 = svd_invdet(sigma.z, PSUEDO_INVERSE_THRESHOLD);

            o[0][0] = v[0][0] * d0 * v[0][0] + v[0][1] * d1 * v[0][1] + v[0][2] * d2 * v[0][2];
            o[0][1] = v[0][0] * d0 * v[1][0] + v[0][1] * d1 * v[1][1] + v[0][2] * d2 * v[1][2];
            o[0][2] = v[0][0] * d0 * v[2][0] + v[0][1] * d1 * v[2][1] + v[0][2] * d2 * v[2][2];
            o[1][0] = v[1][0] * d0 * v[0][0] + v[1][1] * d1 * v[0][1] + v[1][2] * d2 * v[0][2];
            o[1][1] = v[1][0] * d0 * v[1][0] + v[1][1] * d1 * v[1][1] + v[1][2] * d2 * v[1][2];
            o[1][2] = v[1][0] * d0 * v[2][0] + v[1][1] * d1 * v[2][1] + v[1][2] * d2 * v[2][2];
            o[2][0] = v[2][0] * d0 * v[0][0] + v[2][1] * d1 * v[0][1] + v[2][2] * d2 * v[0][2];
            o[2][1] = v[2][0] * d0 * v[1][0] + v[2][1] * d1 * v[1][1] + v[2][2] * d2 * v[1][2];
            o[2][2] = v[2][0] * d0 * v[2][0] + v[2][1] * d1 * v[2][1] + v[2][2] * d2 * v[2][2];
        }

        static void svd_solve_ATA_ATb(NativeArray<float> ATA, float4 ATb, float4* x)
        {
            float3x3 V = new();
            V[0][0] = 1.0f; V[0][1] = 0.0f; V[0][2] = 0.0f;
            V[1][0] = 0.0f; V[1][1] = 1.0f; V[1][2] = 0.0f;
            V[2][0] = 0.0f; V[2][1] = 0.0f; V[2][2] = 1.0f;

            float4 sigma = new();
            svd_solve_sym(ATA, &sigma, V);

            // A = UEV^T; U = A / (E*V^T)
            float3x3 Vinv = new();
            svd_pseudoinverse(Vinv, sigma, V);
            svd_mul_matrix_vec(x, Vinv, ATb);
        }

        static void svd_vmul_sym(float4* result, NativeArray<float> A, float4 v)
        {
            float4 A_row_x = float4(A[0], A[1], A[2], 0.0f);

            (*result).x = dot(A_row_x, v);
            (*result).y = A[1] * v.x + A[3] * v.y + A[4] * v.z;
            (*result).z = A[2] * v.x + A[4] * v.y + A[5] * v.z;
        }

        // QEF
        ////////////////////////////////////////////////////////////////////////////////

        static void qef_add(float4 n, float4 p, NativeArray<float> ATA, float4* ATb, float4* pointaccum)
        {
            ATA[0] += n.x * n.x;
            ATA[1] += n.x * n.y;
            ATA[2] += n.x * n.z;
            ATA[3] += n.y * n.y;
            ATA[4] += n.y * n.z;
            ATA[5] += n.z * n.z;

            float b = dot(p, n);
            (*ATb).x += n.x * b;
            (*ATb).y += n.y * b;
            (*ATb).z += n.z * b;

            (*pointaccum).x += p.x;
            (*pointaccum).y += p.y;
            (*pointaccum).z += p.z;
            (*pointaccum).w += 1.0f;
        }

        static float qef_calc_error(NativeArray<float> A, float4 x, float4 b)
        {
            float4 tmp;

            svd_vmul_sym(&tmp, A, x);
            tmp = b - tmp;

            return dot(tmp, tmp);
        }

        static float qef_solve(NativeArray<float> ATA, float4 ATb, float4 pointaccum, float4* x)
        {
            float4 masspoint = pointaccum / pointaccum.w;

            float4 A_mp = new();
            svd_vmul_sym(&A_mp, ATA, masspoint);
            A_mp = ATb - A_mp;

            svd_solve_ATA_ATb(ATA, A_mp, x);

            float error = qef_calc_error(ATA, *x, ATb);
            (*x) += masspoint;

            return error;
        }

        public static float4 Solve(float4* positions, float4* normals, int count, out float error)
        {
            if (count == 0)
            {
                error = 1;
                return NAN;
            }

            float4 pointaccum = new();
            float4 ATb = new();
            NativeArray<float> ATA = new(6, Allocator.Temp);

            for (int i = 0; i < count; ++i)
            {
                qef_add(normals[i], positions[i], ATA, &ATb, &pointaccum);
            }

            float4 solved_position = new();
            error = qef_solve(ATA, ATb, pointaccum, &solved_position);
            qef_solve(ATA, ATb, pointaccum, &solved_position);

            ATA.Dispose();
            return solved_position;
        }
    }
}